...
namespace Source.Game.HexMerge
{
    public sealed class HexDragTutorialController : MonoBehaviour
    {
        ...
        private bool CanPlaceDraggedStack(int draggableIndex, [Bridge.Ref] HexCoord coord)
        {
            if (_isCompleted)
            {
                return true;
        ...
        private void OnDragPlaced(int draggableIndex, [Bridge.Ref] HexCoord coord)
        {
            if (_isCompleted || draggableIndex != _config.EnabledDraggableIndex)
            {
                return;
        ...
        private Vector2 WorldToCanvasPosition([Bridge.Ref] Vector3 worldPosition)
        {
