...
public sealed class HexBoardModel
{
    ...
    public bool Exist([Bridge.Ref] HexCoord coord)
    {
        var offset = HexMath.ToOffset(coord);
        return offset.x >= 0 && offset.x < Width && offset.y >= 0 && offset.y < Height;
    }

    public Option<HexTile> GetTopTile([Bridge.Ref] HexCoord coord)
    {
    ...
    public void AddTile([Bridge.Ref] HexCoord coord, [Bridge.Ref] HexTile tile, int amount = 1)
    {
        if (amount <= 0)
        {
    ...
    public Option<HexTile> RemoveTopTile([Bridge.Ref] HexCoord coord)
    {
        if (!_cells.Contains(coord))
        {
            return Option<HexTile>.None;
    ...
    private void ApplyMerge([Bridge.Ref] HexMergePlan plan)
    {
    ...
    private static void CollectCellCollapses([Bridge.Ref] HexCoord coord, HexCellModel cell, List<HexCollapsed> collapses)
    {
    ...
    private void EnqueueForMerge([Bridge.Ref] HexCoord coord)
    {
    ...
    private HexCellModel GetCellOrThrow([Bridge.Ref] HexCoord coord)
    {
        if (!_cells.Contains(coord))
        {
    ...
    private void NotifyCellChanged([Bridge.Ref] HexCoord coord)
    {
