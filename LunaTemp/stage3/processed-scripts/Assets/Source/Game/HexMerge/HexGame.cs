...
public sealed class HexGame : MonoBehaviour
{
    ...
    public void PlaceDraggableStack([Bridge.Ref] HexCoord coord, [Bridge.Ref] HexDraggableStackSetup stackSetup)
    {
        if (_isFinished)
        {
            return;
    ...
    private void OnBoardCollapsed([Bridge.Ref] HexCollapsed _)
    {
        EvaluateFinishState();
    }
    ...
    private static HexTileAddition[] BuildTileAdditions([Bridge.Ref] HexCoord coord, [Bridge.Ref] HexDraggableStackSetup stackSetup)
    {
