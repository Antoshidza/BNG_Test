...
namespace Source.Game.HexMerge
{
    public sealed class HexGamePresenter : MonoBehaviour
    {
        ...
        private void OnBoardTileAdded([Bridge.Ref] HexTileAdded added)
        {
            _animationQueue.Enqueue(AnimationEvent.CreateTileAdded(added));
            TryStartAnimationQueue();
        }

        private void OnBoardMerged([Bridge.Ref] HexMergePlan plan)
        {
            _animationQueue.Enqueue(AnimationEvent.CreateMerged(plan));
            TryStartAnimationQueue();
        }

        private void OnBoardCollapsed([Bridge.Ref] HexCollapsed collapse)
        {
        ...
        private void ApplyTileAdded([Bridge.Ref] HexTileAdded added)
        {
        ...
        private DraggableStackRuntime CreateDraggableStackRuntime([Bridge.Ref] HexDraggableStackSetup stackSetup, int slotIndex, [Bridge.Ref] Vector3 position)
        {
        ...
        private Option<HexCoord> GetDropCoord([Bridge.Ref] Vector3 stackPosition)
        {
        ...
        private void UpdateDragCellMarker([Bridge.Ref] Vector3 stackPosition)
        {
        ...
        private void PlaceDraggedStack(DraggableStackRuntime stack, [Bridge.Ref] HexCoord coord)
        {
        ...
        private Vector3 GetDraggableStackWorldPosition([Bridge.Ref] HexCoord coord)
        {
            HexLayout layout = new HexLayout(_hexSize, transform.position);
            return layout.HexToWorld(coord, _tileBaseHeight);
        }

        private Vector3 GetCellWorldPosition([Bridge.Ref] HexCoord coord)
        {
            HexLayout layout = new HexLayout(_hexSize, transform.position);
        ...
        private void StartMergeAnimation([Bridge.Ref] HexMergePlan plan, float batchDelay)
        {
        ...
        private IEnumerator AnimateCollapseBatch(List<HexCollapsed> collapseBatch)
        {
            ...
            for (int batchIndex = 0; batchIndex < collapseBatch.Count; batchIndex++)
            {
                ...
                Vector3 lastCollapseFxPosition = default(UnityEngine.Vector3);
        ...
        private void RepositionCellTiles([Bridge.Ref] HexCoord coord, List<HexTileView> tiles)
        {
        ...
        private Vector3 GetTileWorldPosition([Bridge.Ref] HexCoord coord, int tileIndex)
        {
            var layout = new HexLayout(_hexSize, transform.position);
            return layout.HexToWorld(coord, _tileBaseHeight + tileIndex * _stackHeightStep);
        }

        private HexTileView GetTileFromPool([Bridge.Ref] Vector3 position, [Bridge.Ref] Quaternion rotation)
        {
        ...
        private IEnumerator PlayCollapseVfx([Bridge.Ref] CollapseVfxRequest collapseVfxRequest)
        {
        ...
        private void PlayTileAudio([Bridge.Ref] AudioCue audioCue)
        {
        ...
        private readonly struct AnimationEvent
        {
            ...
            public static AnimationEvent CreateTileAdded([Bridge.Ref] HexTileAdded tileAdded)
            {
                return new AnimationEvent(AnimationEventKind.TileAdded, tileAdded, default(HexMergePlan), default(HexCollapsed));
            }

            public static AnimationEvent CreateMerged([Bridge.Ref] HexMergePlan merge)
            {
                return new AnimationEvent(AnimationEventKind.Merged, default(HexTileAdded), merge, default(HexCollapsed));
            }

            public static AnimationEvent CreateCollapsed([Bridge.Ref] HexCollapsed collapse)
            {
                return new AnimationEvent(AnimationEventKind.Collapsed, default(HexTileAdded), default(HexMergePlan), collapse);
            }
        }
